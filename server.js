...
// ── Base de données Turso (ou SQLite local en fallback) ───────────────────────
const db = createClient(
  process.env.TURSO_URL
    ? { url: process.env.TURSO_URL, authToken: process.env.TURSO_TOKEN }
    : { url: `file:${path.join(DATA_DIR, 'quizzlive.db')}` }
);
...
async function dbRun(sql, args = []) { await db.execute({ sql, args }); }
...
async function getSession(id) {
  const s = await dbGet('SELECT * FROM sessions WHERE id=?', [id]);
  if (!s) return null;
  return {
    ...s,
    participants: JSON.parse(s.participants),
    answers: JSON.parse(s.answers),
    quiz_snapshot: JSON.parse(s.quiz_snapshot),
    results: s.results ? JSON.parse(s.results) : null,
    current_question: Number(s.current_question)
  };
}
async function saveSession(s) {
  await dbRun(
    `UPDATE sessions SET status=?,current_question=?,participants=?,answers=?,results=?,finished_at=? WHERE id=?`,
...
app.post('/api/register', async (req, res) => {
  ...
  if (await dbGet('SELECT id FROM trainers WHERE username=?', [username.trim()])) return res.status(409).json({ error: 'Identifiant déjà pris' });
  const id = uuidv4();
  await dbRun('INSERT INTO trainers(id,username,password_hash) VALUES(?,?,?)', [id, username.trim(), bcrypt.hashSync(password, 10)]);
  res.json({ token: createToken(id), username: username.trim() });
});

app.post('/api/login', async (req, res) => {
  const trainer = await dbGet('SELECT * FROM trainers WHERE username=?', [req.body.username?.trim()]);
  if (!trainer || !bcrypt.compareSync(req.body.password, trainer.password_hash))
...
app.get('/api/quizzes', async (req, res) => {
  ...
  const rows = await dbAll('SELECT * FROM quizzes WHERE trainer_id=? ORDER BY updated_at DESC', [tid]);
...
app.post('/api/quizzes', async (req, res) => {
  ...
  const id = uuidv4();
  await dbRun('INSERT INTO quizzes(id,trainer_id,title,description,questions) VALUES(?,?,?,?,?)',
...
app.put('/api/quizzes/:id', async (req, res) => {
  ...
  if (!tid) return res.status(401).json({ error: 'Non autorisé' });
  const { title, description, questions } = req.body;
  if (!await dbGet('SELECT id FROM quizzes WHERE id=? AND trainer_id=?', [req.params.id, tid]))
    return res.status(404).json({ error: 'Introuvable' });
  await dbRun(`UPDATE quizzes SET title=?,description=?,questions=?,updated_at=strftime('%s','now') WHERE id=?`,
...
app.delete('/api/quizzes/:id', async (req, res) => {
  const tid = authTrainer(req);
  if (!tid) return res.status(401).json({ error: 'Non autorisé' });
  await dbRun('DELETE FROM quizzes WHERE id=? AND trainer_id=?', [req.params.id, tid]);
  res.json({ ok: true });
});
...
app.post('/api/sessions', async (req, res) => {
  const tid = authTrainer(req);
  if (!tid) return res.status(401).json({ error: 'Non autorisé' });
  const quiz = await dbGet('SELECT * FROM quizzes WHERE id=? AND trainer_id=?', [req.body.quizId, tid]);
  ...
  while (await dbGet("SELECT id FROM sessions WHERE pin=? AND status != 'finished'", [pin]));
  const id = uuidv4();
  const snapshot = { id: quiz.id, title: quiz.title, description: quiz.description, questions };
  await dbRun('INSERT INTO sessions(id,pin,quiz_id,trainer_id,quiz_snapshot) VALUES(?,?,?,?,?)',
    [id, pin, quiz.id, tid, JSON.stringify(snapshot)]);
  res.json({ id, pin });
});

app.get('/api/sessions/by-pin/:pin', async (req, res) => {
  const s = await dbGet("SELECT id,pin,status,quiz_snapshot FROM sessions WHERE pin=? AND status != 'finished'", [req.params.pin]);
...
app.get('/api/sessions/history', async (req, res) => {
  ...
  const rows = await dbAll(
    `SELECT s.id,s.pin,s.status,s.created_at,s.finished_at,s.results,q.title as quiz_title
...
// WebSocket
// ════════════════════════════════════════════════════════════════════════════════
async function revealAnswer(sessionId, qIndex) {
...
const handlers = {
  async host_join(ws, { sessionId, token }) {
  ...
  async player_join(ws, { sessionId, name }) {
    const s = await getSession(sessionId);
  ...
  async player_answer(ws, { qIndex, answer, timeRatio }) {
  ...
  async next_question(ws) {
    const { sessionId } = ws.meta;
    const s = await getSession(sessionId);
    if (!s || s.status !== 'scoreboard') return;
    const nextQ = s.current_question + 1;
    s.current_question = nextQ; s.status = 'question'; s.answers[nextQ] = {};
    await saveSession(s);
